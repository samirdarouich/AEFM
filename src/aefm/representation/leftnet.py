...
from aefm.representation.core import MLP
...
class NeighborEmb(MessagePassing):
    ...
    def forward(self, s, edge_index, embs):
        s_neighbors = self.ln_emb(s)
...
class LEFTNet(torch.nn.Module):
    ...
    def forward(
        ...
        i = inputs[properties.idx_i]
        j = inputs[properties.idx_j]
        pos = inputs[properties.R]

        subgraph_mask = inputs.get(properties.subgraph_mask, None)

        edge_index = torch.stack([i, j])

        # embed z, assuming last column is atom number
        z_emb = self.embedding(atomic_numbers)
        ...
        # init node features
        s = self.neighbor_emb(z_emb, edge_index, f)
        ...
        for i in range(self.num_layers):
            ...
            # Dropout on scalar features
            s = self.dropout(s)
            
        # collect results
        inputs["scalar_representation"] = s
        inputs["vector_representation"] = vec
