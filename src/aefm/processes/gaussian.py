...
from aefm.noise_schedules import NoiseSchedule

from .base import DiffusionProcess, FlowProcess
...
class GaussianCFM(FlowProcess):
    """
    Base class for flow matching models using conditional Gaussian probability paths.
    """

    @abstractmethod
    def compute_mu_t(self, x_0: torch.Tensor, x_1, t: torch.Tensor) -> torch.Tensor:
        """
        Compute the mean of the conditional probability path N(mu_t, sigma_t).

        Args:
            x_0: input tensor x_0 ~ p_0.
            x_1: input tensor x_1 ~ p_1.
            t: time step.
        """
    ...
    def compute_sigma_t(self, t: torch.Tensor) -> torch.Tensor:
        """
        Compute the standard deviation of the conditional probability path
        N(mu_t, sigma_t).
    ...
    def compute_conditional_flow(
        ...
        Args:
            ...
            x_1: input tensor x_1 ~ p_1.
            t: time step.
            x_t: Samples drawn from probability path pt.
        """
        raise NotImplementedError
    ...
    def sample_xt(
        ...
        """
        Draw a sample from the conditional probability path N(mu_t, sigma_t).

         Args:
            x_0: input tensor x_0 ~ p_0.
            x_1: input tensor x_1 ~ p_1.
            t: time step.
            epsilon: Noise sample from N(0, 1) and COM free.

        Returns:
            Tensor: Sample from probability path.
    ...
    def flow(
        ...
        **kwargs,
        ...
        """
        Flow between x_0 and x_1 by time t. Sample from conditional Gaussian probability
        path with mu_t and sigma_t. Return tuple of tensors x_t and v_t, or Dict of
        tensors with x_t and v_t.
        ...
        Args:
            ...
            t: time step.
            return_dict: if True, return results under a dictionary of tensors. Defaults
                to False.
            sample_key: key for the flowed sample at time t.
            velocity_key: key for the conditional vector field at time t.
        """
        ...
        eps = sample_noise_like(x_0, self.invariant, idx_m)
        x_t = self.sample_xt(x_0, x_1, t, eps)
        u_t = self.compute_conditional_flow(x_0, x_1, t, x_t)

        if return_dict:
            return {sample_key: x_t, output_key: u_t}
        else:
            return x_t, u_t


class OTCFM(GaussianCFM):
    """
    Class for conditional flow matching methods. This class implements the conditional
    flow matching methods from:
        [1] Improving and Generalizing Flow-Based Generative Models with minibatch
            optimal transport, Preprint, Tong et al.
    ...
    def __init__(self, sigma: float, **kwargs):
        ...
        Args:
            sigma: Sigma of Gaussian probability path. Helps to stabilize the neural
                network in inference, by adding noise to the samples drawn from the
...
class VFM(OTCFM):
    ...
    def compute_conditional_flow(
        ...
        """
        Compute the conditional vector field ut(x1|x0) = x1.
...
class GaussianDDPM(DiffusionProcess):
    ...
    def diffuse(
        ...
        **kwargs,
        ...
        if return_dict:
            return {sample_key: x_t, output_key: noise}
        else:
            return x_t, noise
    ...
    def reverse_step(
        ...
        **kwargs,
        ...
        if self.invariant and idx_m is not None:
            noise = batch_center_systems(noise, idx_m, dim=-2)
