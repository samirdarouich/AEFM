    ...
    for props in tqdm(dataloader):
        ...
        for p in property_names:
            ...
            if atomref and p in atomref.keys():
                ...
                if structure.conditions_idx_m in props:
                    idx_m = props[structure.conditions_idx_m]
        ...
        n_atoms = props[structure.n_atoms]
        if structure.conditions_n_atoms in props:
            n_atoms = props[structure.conditions_n_atoms]
        norm = norm_mask[:, None] * n_atoms[None, :] + (1 - norm_mask[:, None])
        sample_values /= norm
